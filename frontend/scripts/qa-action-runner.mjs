...
for (const [source, pattern, label] of [
  ...
  [operationalSource, /ACTION_FEEDBACK\.scheduleSaved[\s\S]*\(\) => setRule\(emptyScheduleRule\(rule\.effective_from \|\| today\)\)/, 'Escala salva deve limpar rule e weekdays via onSuccess'],
  [operationalSource, /ACTION_FEEDBACK\.scheduleRuleRemoved[\s\S]*setRule\(emptyScheduleRule\(\)\)/, 'Remocao de escala deve limpar rule e weekdays quando aplicavel'],
